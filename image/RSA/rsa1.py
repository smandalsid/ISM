...
if n==1:
    ...
    ciphertext=encrypt(plaintext, 'image/RSA/public_key.pem')
    print("\nEncrypted message: ")
    print(ciphertext)
    ...
    # src='RSA/img.png'
    # dest='RSA/stego.png'
    Encode(src, ciphertext, dest)
else:
    dest=input("Enter name of image file: ")
    ciphertext2=Decode(dest)
    print("\nDecoded message from image: ")
    print(bytes(ciphertext2))
    decrypted=decrypt('image/RSA/private_key.pem', ciphertext2)
    print("\nDecrypted message: ")
    print(decrypted)
